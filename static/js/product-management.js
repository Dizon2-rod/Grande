...
function initProductForm() {
    ...
    const categoryInput = document.getElementById('productCategory') || document.querySelector('input[name="category"]');
    (async () => {
      try {
        if (categoryInput) {
          ...
          if (data.success && data.categories && data.categories.length > 0) {
            // Set the seller's registered category (first one if multiple)
            const category = data.categories[0];
            categoryInput.value = category;
            
            // Update size options based on category
            updateSizeOptionsForCategory(category);
    ...
    function attachSizePillListeners() {
      ...
      if (categoryInput && categoryInput.value === 'Shoes & Accessories') {
        sizePills = shoeSizesContainer ? shoeSizesContainer.querySelectorAll('.size-pill') : [];
      } else {
        sizePills = clothingSizesContainer ? clothingSizesContainer.querySelectorAll('.size-pill') : [];
      }
